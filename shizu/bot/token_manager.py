...
class TokenManager(Item):
    ...
    async def _find_bot(self) -> Union[str, None]:
        """Найти бота"""
        async with fsm.Conversation(self._app, "@BotFather", True) as conv:
            logging.info("Проверка наличия бота...")
            ...
            if not buttons_text:
                logger.error("The bot will not find, an attempt to create a new bot...")
                return False
            
            buttons = [index for index, item in enumerate(buttons_text) if "shizu" in item.lower()]
            logger.success("Найден бот %s", buttons_text)
    ...
    async def _create_bot(self) -> Union[str, None]:
        ...
        async with fsm.Conversation(self._app, "@BotFather", True) as conv:
            ...
            if any(phrase in response.text for phrase in error_phrases):
                logging.error("An error occurred when creating the bot. @BotFather's response:")
            ...
            await conv.ask(f"🐙 Shizu UserBot of {utils.get_display_name(self._all_modules.me)[:45]}")
            await conv.get_response()
